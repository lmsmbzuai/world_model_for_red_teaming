...
- (3) LLM + Reasoning Loop + World Model

When you run the environment, the messages exchanged between agents are displayed in the terminal.
If you use architecture (2) or (3), a JSON file will be generated in the project’s root directory at the end of execution. This file contains the reasoning traces and world model predictions.
...
The experiment can be reproduced, but note that it may take more than 3 hours to complete, even when using a GPU. The procedure is as follows, based on the same setup described in the Installation & Usage section.
